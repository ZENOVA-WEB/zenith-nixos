...

{
  ...
  home.activation.syncDotfilesAndShell = lib.hm.dag.entryAfter [ "onFilesChange" ] ''
    ...
    if [ ! -d "$HYPR_DIR" ]; then
      ...
      if [ -d "$HYPR_DIR/.git" ]; then
        $DRY_RUN_CMD $GIT -C "$HYPR_DIR" pull --ff-only || true
      fi
    fi
    ...
    if [ ! -d "$QUICKSHELL_DIR" ]; then
      ...
      if [ -d "$QUICKSHELL_DIR/.git" ]; then
        $DRY_RUN_CMD $GIT -C "$QUICKSHELL_DIR" pull --ff-only || true
      fi
    fi
  '';
}